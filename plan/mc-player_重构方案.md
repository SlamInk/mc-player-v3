...
| 上游依据 | `doc/mc-player_架构设计文档_v3.0.md` / `doc/mc-player_ADR.md` / `doc/mc-player_性能量度规范.md` / `doc/hardware-decode-dependencies.md` / `CLAUDE.md` |
| 落地节奏 | 11 个阶段（Phase 0 ~ Phase 10），每阶段 = 一次 `git commit + push` |
...
1. detector：`IDXGIAdapter1::GetDesc1` 拿 VendorId；按 ADR-016 §5.6.6 分发到对应 SDK 的预期文件存在性 probe（含 `%LOCALAPPDATA%` cache + System32）。
...
|---|---|
| `mc.e2e.client_internal_p95`（LAN + NVDEC + 240Hz VRR + zerolatency 编码 + 5 子目标全命中） | ≤ 8ms |
...
```
feat(phase-9): Capability Probe Suite + Preset 架构基座（ADR-017 ~ ADR-019）

- probe/hardware_probe: 复用现有 probe 重构进统一 HardwareSnapshot 接口
...
- preset/preset_selector: ADD §7.5.4 匹配算法 → 5 档 active_preset_id
- preset/preset_apply: 一次性 apply 到 6 子系统（decoder/jitter/render/present/rtcp/gate）
- controller: 启动序列改造，BOOTSTRAP → probe → reload to active preset
- 子目标命中（按需勾）：[ ] DCOMP NV12 直显 [ ] RFC 5506 [ ] ZeroJitter [ ] race_to_display [ ] CUDA Graphs

before/after（NVIDIA + 240Hz VRR + LAN）：
- mc.preset.active_id:                   <无字段> → SDI_REPLACEMENT
- mc.preset.bootstrap_to_active_ms:                → 800ms
- mc.e2e.client_internal_p95:            22ms     → 8ms (子目标全命中)
                                                    14ms (子目标 0 命中)
...
| `mc-player/src/codec/codec_bridge.cpp` | decoder hint reload（NVDEC ulMaxDisplayDelay / oneVPL AsyncDepth / AMF REORDER_MODE）的 GOP-aligned 切换闸 |
...
- codec/codec_bridge: decoder hint GOP-aligned 切换
...
| ADR-017 Capability-Driven Preset 架构 | 三维 probe + 5 档 preset selector + 一次性 apply 到 6 子系统 |
...
| `mc.e2e.client_internal_p95` | SDI_REPLACEMENT preset ≤8ms（5 子目标全命中）；REALTIME_LAN ≤20ms（ADD §8.4，warm_steady） |
...
| `preset/live_reload.cpp::downgrade_dispatch / upgrade_loop` | ADD §7.5.5 / ADR-020 / capability_probe 设计 §8 |
