...
void FrameValidityGate::record_drop_locked(uint32_t missing_mask, int64_t pts_us) noexcept {
    if (missing_mask & kValidityRefs)     drops_refs_.fetch_add(1,     std::memory_order_relaxed);
    if (missing_mask & kValidityParams)   drops_params_.fetch_add(1,   std::memory_order_relaxed);
    if (missing_mask & kValidityRecovery) drops_recovery_.fetch_add(1, std::memory_order_relaxed);
    if (missing_mask & kValidityColor)    drops_color_.fetch_add(1,    std::memory_order_relaxed);
    if (missing_mask & kValidityReorder)  drops_reorder_.fetch_add(1,  std::memory_order_relaxed);
    if (missing_mask & kValidityFence)    drops_fence_.fetch_add(1,    std::memory_order_relaxed);
