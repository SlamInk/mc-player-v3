...
[2] DXVA-direct (ID3D11VideoDevice)                    ← 跨 vendor 通用，host 抽象层数 1
       ...
       不依赖 MFT category 注册
                   ↓ 失败 / profile 不支持
[3] CodecMftVideo (Windows MFT)                        ← OS 标准抽象兼容档，host 抽象层数 2
       优先 hw_url=1 && async=1（hardware async，ADR-002 标准约束档）
       回退 hw_url=0 && async=0（Microsoft sync software MFT）— IoT LTSC
       裁剪 MediaPlayback 时的功能性兜底，延时 +100ms 但优于黑屏
                   ↓ 全部失败
...
| 3 | `mfh264dec.dll` / `mfh265dec.dll`（OS software MFT） | OS / Microsoft HEVC Extension | 缺失则档 3 sync software 兜底也不可用；IoT LTSC 默认裁剪（见 §2.1） |
...
### 2.1 实测：IoT Enterprise LTSC 2024 (build 26100) 缺失组件清单

本仓库主开发机的实测结果（参考样本，非全量普查）：

| 组件 | 路径 / 注册项 | 状态 |
|---|---|---|
| `mfh264dec.dll` | `%WINDIR%\System32\mfh264dec.dll` | **缺失** |
| `mfh265dec.dll` | 同上 | **缺失** |
| `msmpeg2vdec.dll` | 同上 | **缺失** |
| MFT decoder category | `HKLM:\SOFTWARE\Microsoft\Windows Media Foundation\Transforms\Categories\{d6c02d4b-…}` | **不存在** |
| Intel `igd11dxva64.dll` | 由 GPU driver 安装 | 存在（DXVA DDI 路径完整） |
| `d3d11.dll` / `dxgi.dll` | OS 内置 | 存在 |
| `Microsoft.HEVCVideoExtension` Appx | — | 未装 |

按 ADR-015 评估：
- 档 3 hardware async MFT：`MFTEnumEx(ALL)` 返 0（category key 不存在）→ 跳过
- 档 3 sync software MFT 兜底：mfh264dec.dll 缺失 → 也无法启动
- 档 2 DXVA-direct H.265：`CheckVideoDecoderProfile` 返 1，**实测可用**
- 档 2 DXVA-direct H.264：`CheckVideoDecoderProfile` 返 1，**实测 silent fail**（详见 §4.2）
- 档 4 mc-libcodec：始终可用

**修复后**（装 Intel oneVPL runtime 或 Enable MediaPlayback + Intel hw MFT）：档 1 oneVPL 与档 3 hardware async 均可达。

### 2.2 整组 MediaPlayback feature 默认裁剪
...
- 当前 `codec_dxva_video.cpp` 实装范围：HEVC Main / Main10 默认启用；H.264 默认走端到端 capability self-test 决策（见 §4.2.1）。

档 2 启用后 stats 上 `decoder_kind=DXVA_DIRECT`，与档 1 同 GPU 解码单元、host 侧多 1 层 D3D11 video API。

#### 4.2.1 H.264 capability-then-select：解决 driver silent fail

**为什么不能仅靠 `CheckVideoDecoderFormat` 表面 supported flag**：Intel UHD 730 + Win11 IoT LTSC 26100 实测，`CheckVideoDecoderProfile` / `CheckVideoDecoderFormat` 均返 TRUE，但跑实际 H.264 流时 driver `SubmitDecoderBuffers` 返 OK，输出的 NV12 DPB 全 0（或全 128 driver 中性灰），表现为应用持续黑屏 / 单一灰场。同硬件 ffmpeg `-c:v h264 -hwaccel d3d11va` 可正常解码——driver 能力存在，但暴露的 PicParams 字段语义与 mc-player 实装存在某个待定位的 mismatch（无 GPL-clean DXVA H.264 reference 可对照）。

**对策**：`probe_dxva_h264_capable(ID3D11Device*)` （`mc-player/src/media/codec_dxva_video.cpp`）— controller 在档 2 H.264 启动前跑一帧 64×64 baseline IDR fixture 端到端 self-test：

1. 创建 `ID3D11VideoDecoder` 用 `H264_VLD_NoFGT` profile + `ConfigBitstreamRaw=1` config
2. 把 61 字节 fixture（SPS+PPS+IDR slice，全黑 64×64）塞进 bitstream / picparams / slicectrl 三个 buffer
3. `DecoderBeginFrame` → `SubmitDecoderBuffers` → `DecoderEndFrame`
4. CopySubresource 取出 NV12 staging texture，读回 Y plane 8×64 像素
5. 全黑 IDR Y 应 ≈ 16（BT.709 limited 黑值），允许 8–64 容差。Y 全 0 / 全 128 都判 silent fail

判定 `capable=true` 才创建 `CodecDxvaVideo`；`capable=false` 直接 `record_skip(2, "self_test_fail_silent_decode")` 并降 tier 3。

**env 覆盖**：

| 环境变量 | 行为 |
|---|---|
| 未设置 | 默认走 self-test（推荐） |
| `MCP_ENABLE_DXVA_H264=1` | 跳过 self-test 强制启用（诊断用） |
| `MCP_ENABLE_DXVA_H264=0` | 跳过 self-test 强制禁用（对照测试 tier 3） |
| `MCP_SKIP_TIER2=1` | 整档 2 跳过 |

**启动 log 示例**：

```
Controller: tier2 DXVA H.264 capability self-test capable=0
CodecDxvaVideo: H.264 self-test NV12 Y[8x64] sum=0 min=0 max=0 → capable=0 (全 0/128 = driver silent fail;~16 = 全黑 IDR 解码成功)
Controller: tier3 MFT sync software (IoT LTSC fallback) active
```

H.265 路径无 silent fail 已知现象（HEVC profile 与 PicParams 语义经过更早的 reference 实装锚定），暂不上 self-test。
...
5. **关键**：仅有 `Microsoft H264 Video Decoder MFT` (sync software) 也算可用——ADR-002 仅约束档 3 的 hardware async 路径必须 async；sync software 作为 IoT LTSC 兜底已被 controller 接受为档 3。但 reorder>0 流上 +100 ms 延时，性能上仍鼓励安装 hw MFT
...
- 档 2 `codec_dxva_video.cpp` HEVC Main / Main10 默认启用；H.264 实装存在但 Intel UHD 730 + IoT LTSC 上 silent fail，default 行为由 capability self-test 决定（§4.2.1）。
- 档 1 vendor SDK 三家 (NVDEC / oneVPL / AMF) 全部在路线规划。
- 档 3 sync software MFT 已作为 IoT LTSC 兜底被接受（§4.3.5）；hardware async MFT 仍是档 3 性能首选。
- 短期优先级：档 2 H.264 driver silent fail 根因定位 → 档 1 NVDEC（最广覆盖）→ 档 1 oneVPL → 档 1 AMF。
...
- **ADR-001（MFT 缩窄为 OS 标准抽象兼容档）**：档 3 优先 hw_url=1 && async=1；sync software 仅作 IoT LTSC 兜底（§4.2.1 / §4.3.5）。
- **ADR-002（硬件 MFT 永远 async）**：**仅约束档 3 hardware MFT 路径**（hw_url=1 && async=1 时必须 unlock async）；sync software MFT 不在 ADR-002 约束范围。
...
- metric 字段名、阈值、CI 闸定义统一在 [`mc-player_性能量度规范.md`](./mc-player_性能量度规范.md)：本文档相关的 metric 集中在性能规范 **§2.4 caps_probe / open** + **§6 解码档位与降级** + **§10.5 档位下沉排障**。运维监控 `mc.decoder.kind` 分布与 `mc.decoder.tier_drift` 即可发现"档位下沉"问题；`mc.decoder.kind=LIBCODEC + mc.stage.decode_actual_p95 > 25ms` 触发"档 4 + 性能不达标"告警。
- 安装文档（README）按场景给指导：
