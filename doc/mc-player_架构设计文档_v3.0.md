...
| 维度 | 目标 | 前提 |
|---|---|---|
| 客户端内部延时（按 ADR-017 Preset 分档，warm_steady p95）| SDI_REPLACEMENT 5~8ms（5 子目标全命中）/ REALTIME_LAN 8~24ms / STREAMING_WIFI 15~46ms / WAN_FALLBACK 60~190ms | 硬件 ∩ 网络 ∩ 编码器 三维 capability 联合命中对应 preset；详见 §3.5 / §8.4 |
...
mc-player 的子系统决策不是"每个模块自己按 ADR 静态选档"，而是**三维 capability probe（硬件 ∩ 网络 ∩ 编码器）联合驱动一个 Preset，由 Preset 协同配置整个管线**。这是与"先 mc_open 再各自硬绑参数"传统播放器架构最大的区别——决策点统一、子系统协同优化、运行期可热切。
...
- **档 1（vendor SDK）**：NVDEC 用 `CUVIDPARSERPARAMS::ulMaxDisplayDelay = 0` + 每包 `CUVID_PKT_ENDOFPICTURE`；oneVPL 用 `AsyncDepth = 1` + `mfxBitstream.DataFlag |= MFX_BITSTREAM_COMPLETE_FRAME`；AMF 用 `AMF_VIDEO_DECODER_REORDER_MODE = AMF_VIDEO_DECODER_MODE_LOW_LATENCY`。
...
#### 5.6.6 Vendor SDK 内置下载面板（ADR-016 实现）

档 1 启用前置条件是对应 vendor SDK redistributable 已存在。三家 redistributable 各自 ~30-100 MB，全 bundle 让安装包体积翻 3-4 倍且对单 vendor 用户冗余；纯运行期"首次缺失就阻塞"又延迟首次启动 UX。综合方案：**应用内置下载面板，按运行期 detect 的 GPU vendor 引导用户从 vendor 官网下载、本地缓存、后续 probe**。

**面板状态机**：

```
首次启动:
  detect_gpu_vendor() → {NVIDIA, Intel, AMD, Unknown}
  ├─ Unknown / 多卡共存 / SDK 已存在 → 跳过面板，直接启用对应档
  └─ vendor 已知且 SDK 缺失:
      显示 "Hardware decode boost available"
      默认: 跳过 (走档 2 DXVA-direct，仍能放)
      用户选 "下载并启用" → 后台下载 → SHA-256 校验 → 缓存到
        %LOCALAPPDATA%\mc-player\sdk\<vendor>\<version>\
      下次启动 probe 此目录 → 命中即启用档 1
```

**实现约束**：

- 下载源限定 vendor 官网直链（`developer.nvidia.com` / `intel.com/oneapi` / `gpuopen.com/amf`）；不引第三方 redistributable 仓库。
- 校验 SHA-256 + 数字签名（vendor PE 文件均带 Authenticode 签名，校验签发者证书链）。
- 缓存路径用户可见可清理；卸载 mc-player 时按用户偏好保留或清理。
- 离线场景或下载失败 → 静默降到档 2，不阻断启动。

**与 ADR-005/009 的对比**：libdatachannel / libopus 是 baseline 能力（无之则 WHEP / Opus 不可用），bundle 合理；vendor SDK 是**可选性能增强档**——无之则降到档 2 仍能放，按需下载更对应其角色。
...
新策略按 ADR-015 四档降级链处理 HEVC Extension 缺失：

1. **档 1（vendor SDK）** —— GPU 厂商 SDK 自带 H.265 解码能力（NVDEC / oneVPL / AMF），不依赖 OS 端 HEVC Extension。SDK 已存在则直接走档 1。
2. **档 2（DXVA-direct）** —— 大多数 GPU driver 即使 OS 缺 HEVC Extension 仍暴露 `D3D11_DECODER_PROFILE_HEVC_VLD_MAIN` / `_MAIN10` 的 DXVA-DDI。CheckVideoDecoderProfile 通过即可硬解，不依赖 MFT category 注册。这是 IoT LTSC / 缺 MediaPlayback feature 场景下的真兜底。
3. **档 3（MFT hardware async）** —— HEVC MFT 枚举为空时跳过此档（包括 Microsoft HEVC Extension 与 vendor 提供的 hardware MFT）。
4. **档 4（mc-libcodec H.265 软解，§5.7）** —— 前三档全失败时启用，CPU 上升至 ~18%。

stats 暴露 `decoder_kind` ∈ {`VENDOR_SDK_NVDEC`, `VENDOR_SDK_ONEVPL`, `VENDOR_SDK_AMF`, `DXVA_DIRECT`, `MFT_HARDWARE`, `LIBCODEC`} 的真实档位，用户视角无感（H.265 流照常播放）；运维可定位"为什么没走到更高档"——档 1 缺 SDK 走 ADR-016 下载面板补；档 2 不可用一般是 driver 老旧需更新。
...
启动序列与 RTSP 握手 / WHEP ICE **并行**（不阻塞首帧），三个 probe 输出聚合到 `CapabilitySnapshot`：

```
T0=0    Hardware Probe 开始     ┐
T0=0    Network Probe 开始      ├ 并行
T0=0    Signaling 握手开始      ┘
...
> Probe 完成前播放器先按 `BOOTSTRAP` 默认配置（保守版 REALTIME_LAN）启动；首 GOP 后第一次 reload 切换到正式 preset，对用户表现为"前 0.5~1 秒延时偏高，之后稳定到目标值"。
...
```
if  hw.tier ∈ {VENDOR_SDK_NVDEC, VENDOR_SDK_ONEVPL, VENDOR_SDK_AMF}
 && net.link_kind == LAN_SWITCHED && net.rtt_p95 < 1ms && net.loss_rate < 0.01%
 && enc.reorder_depth == 0 && enc.is_zerolatency
 && render.profile == DCOMP_INDEPENDENT_FLIP && render.refresh_rate >= 240 && render.vrr
 then  active = SDI_REPLACEMENT

elif hw.tier ∈ {VENDOR_SDK_*, DXVA_DIRECT}
  && net.link_kind ∈ {LAN_SWITCHED, LAN_WIFI} && net.rtt_p95 < 5ms
  && enc.reorder_depth == 0
  && render.profile ∈ {DCOMP_INDEPENDENT_FLIP, DCOMP_BEST_EFFORT}
  then active = REALTIME_LAN

elif hw.tier ∈ {VENDOR_SDK_*, DXVA_DIRECT, MFT_HARDWARE}
  && net.link_kind ∈ {LAN_WIFI, WAN_WIRED} && net.rtt_p95 < 20ms
  then active = STREAMING_WIFI

elif net.link_kind ∈ {WAN_WIRED, WAN_WIRELESS} || net.loss_rate < 3%
  then active = WAN_FALLBACK

else
  active = SAFE_MODE  // 任何探测失败或 hw.tier == LIBCODEC + 公网，全档保守值
```

**Preset 应用范围**（一次 apply 横跨 6 个子系统）：

| 子系统 | SDI_REPLACEMENT | REALTIME_LAN | STREAMING_WIFI | WAN_FALLBACK |
|---|---|---|---|---|
| Decoder（vendor SDK low-latency hint） | NVDEC `ulMaxDisplayDelay=0` + `EOP` / oneVPL `AsyncDepth=1` / AMF `REORDER_MODE=LOW_LATENCY` | 同左 | LowLatency 但允许 1 reorder | LowLatency 关闭，正常排序 |
| Jitter Buffer | ZeroJitter mode（target 0~1ms） | Kalman aggressive（target 5ms） | Kalman normal（target 15ms） | Kalman safe（target 80ms） |
| Render Profile | DCOMP_INDEPENDENT_FLIP + NV12 直显 + ALLOW_TEARING（VRR） | 同左 | DCOMP_BEST_EFFORT 或 SWAPCHAIN_FLIP | SWAPCHAIN_FLIP |
| Present 调度 | race_to_display（一就绪即 Present） | race_to_display | vsync-aligned | vsync-aligned |
| RTCP 反馈 | RFC 5506 Reduced-Size + AVPF Immediate `trr-int=0` | 同左 | AVPF Immediate | AVPF Regular |
| Frame Validity Gate | refs / params / recovery / reorder strict；color / fence 见下注 | refs / params / recovery / reorder / fence strict；color strict | 六 bit 全 strict | 六 bit 全 strict |

> ⚠️ ADR-014 "正确性先于延时"在 4 个核心 bit（refs / params / recovery / reorder）上**永远 strict**——任何 preset 不让步，因为这 4 bit 的失败直接表现为花屏 / 解码失败 / 帧序错乱。
>
> color 与 fence 子 bit 的语义是"**该 preset 是否走需要它们的代码路径**"，不是"是否放宽正确性"：
> - SDI_REPLACEMENT 走 DCOMP NV12 直显（绕开 shader RGB 转换 + dual-bind），**该路径不依赖 fence 等待 SRV 完成 + 不消费 color metadata**——这两个 bit 在该 preset 下不参与决策，并非放宽。
> - 其余 preset 走 dual-bind shader RGB 转换，必须 strict。
>
> 这与 ADR-017 "Gate 严格度可被 Preset 影响（极端低延时 preset 可放宽 fence/color 子集）"严格一致——"放宽"指代码路径不依赖，而非正确性让步。
...
**关键洞察**：
- SDI_REPLACEMENT preset 端到端 p95 ≤25ms，**逼近消费 PC + 通用 H.264 codec 的物理极限**——超出此值需要硬件协同（GPUDirect RDMA NIC / ST 2110-22 JPEG-XS / SDI 直传）。
