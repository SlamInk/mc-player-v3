...
stats 暴露 `decoder_kind` ∈ {VENDOR_SDK, DXVA_DIRECT, MFT_HARDWARE, LIBCODEC} 的真实档位，用户视角无感（H.265 流照常播放）；运维可定位"为什么没走到更高档"——档 1 缺 SDK 走 ADR-016 下载面板补；档 2 不可用一般是 driver 老旧需更新。
