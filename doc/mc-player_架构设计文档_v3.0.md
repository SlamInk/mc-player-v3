...
|---|---|
| `SDI_REPLACEMENT` | 档 1 vendor SDK + LAN-switched (RTT p95 < 1ms, loss < 0.01%) + 0 reorder + zerolatency hint + 240Hz VRR + DCOMP_INDEPENDENT_FLIP |
| `REALTIME_LAN` | 档 1 vendor SDK / 档 2 DXVA-direct + LAN (RTT p95 < 5ms) + 0 reorder + DCOMP profile |
| `STREAMING_WIFI` | 档 1 / 档 2 / 档 3 MFT_HARDWARE + LAN_WIFI / WAN_WIRED + RTT p95 < 20ms |
| `WAN_FALLBACK` | link_kind ∈ {WAN_WIRED, WAN_WIRELESS} 或 loss_rate 边界条件（**注:见 capability_probe §6.2 的 elif 与设计意图说明**） |
...
| Render Profile | SDI/REALTIME → DCOMP_INDEPENDENT_FLIP + NV12 直显;STREAMING → DCOMP_BEST_EFFORT;WAN → SWAPCHAIN_FLIP |
