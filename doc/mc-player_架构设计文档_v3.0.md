...
| 维度 | 目标 | 前提 |
|---|---|---|
| 客户端内部延时 | 8~25 ms | 硬解档延时 + DCOMP（best-effort）|
...
| 端到端延时 | 60 Hz: 60~110 / 144 Hz VRR: 35~75 / 240 Hz VRR + DCOMP: 25~50 ms | 编码端 low-delay + ADR-015 档 1/2 命中；详见 §8.2 |
...
> **延时基线说明**：四档之间的差异主要来自 host 侧抽象层数（vendor SDK = 0 层 / DXVA-direct = D3D11 video API 1 层 / MFT = MFT 协议 + DXVA 2 层 / libcodec = CPU 全软解），硬件 decode unit 在前三档相同。"极限低延时"目标要求自动选到当前系统能落地的最高档；档位降级在 stats 上明示 (`decoder_kind`)，便于运维定位。
...
- **NVIDIA：NVDEC（Video Codec SDK）** ——`cuvidParseVideoData` 推送 Annex-B；`cuvidDecodePicture` 触发硬件解码；`cuvidMapVideoFrame` 拿到 CUDA device pointer 后通过 `cudaGraphicsD3D11RegisterResource` 与 mc-player 的共享 D3D11 NV12 texture 互通，避免 CUDA→D3D 拷贝。NVDEC driver 内部维持 4 帧 pipeline 但首帧立即解码（NVIDIA 文档明示 "this pipeline does not imply any decoding delay"）；启用 `CUVID_PKT_ENDOFPICTURE` 标记每个 AU 边界以最小化 reorder buffer。
- **Intel：oneVPL（原 Media SDK 后继）**——`MFXVideoDECODE_DecodeHeader` 解析 SPS/PPS；`MFXVideoDECODE_DecodeFrameAsync` 推帧；`mfxFrameSurface1` 直接拿到 D3D11 texture handle（VPL 内部用 D3D11VA）。开 `mfxExtDecVideoProcessing` + `LowDelayBRC` 等 Intel 专属 extension 控制 reorder。
- **AMD：AMF（Advanced Media Framework）**——`AMFComponent::Submit/QueryOutput` 异步管线；`AMFSurface` 拿到 D3D11 texture；启用 `AMF_VIDEO_DECODER_LOW_LATENCY` 属性。
...
**激活原则**：**硬件 MFT 永远 async**（Microsoft Learn "Hardware MFTs" 原文 "All hardware-based MFTs are required to be asynchronous MFTs"——协议要求而非性能选择；sync 调用模式不被支持），激活时显式 unlock async + 用事件生成器驱动；通过 `IMFDXGIDeviceManager` 把 renderer 的 `ID3D11Device` 注入 MFT 确保同 device。**sync software MFT 不在本档接受范围**——它由 ADR-015 直接降到档 4。
...
- **档 1（vendor SDK）**：NVDEC 用 `CUVID_PKT_ENDOFPICTURE` + 单帧 buffering 提示；oneVPL 用 `mfxExtCodingOption2.LookAheadDepth=0` + `LowDelayBRC`；AMF 用 `AMF_VIDEO_DECODER_LOW_LATENCY=true`。
...
- **档 1（vendor SDK）**：NVDEC 关闭 `CUVID_PKT_ENDOFPICTURE` 标记之后的强制 emit 提示，按 SPS reorder 配置 driver 内部 picture buffer 深度；oneVPL 抬高 `mfxVideoParam.AsyncDepth`；AMF 关闭 `AMF_VIDEO_DECODER_LOW_LATENCY`。
