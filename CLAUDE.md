...
- `docs/mc-player_架构设计文档_v3.0.md` — 五层架构 / 数据流 / 线程视图 / 延时预算（ADD）。
- `docs/mc-player_ADR.md` — 23 条 ADR；ADR-014（Frame Validity Gate + Present Epoch）/ ADR-015（解码四级降级链）/ ADR-017（Capability-driven Preset）是必读核心。
- `docs/mc-player_性能量度规范.md` — metric 命名、phase 标签（`cold_start` / `warm_steady` / `error_recovery`）、CI 阈值。**CI 闸只对 `warm_steady` 设阈**。
- `docs/mc-player_capability_probe_设计.md` — 四维 capability probe 的字段与 selector 算法。
- `docs/mc-player_hdcm_设计.md` — Hardware Decode Component Manager（vendor SDK / OS feature / driver 的运行期获取）。
