...
支持 VCL 单 NAL（0-31）、AP 聚合（48）、FU 分片（49）；缓存 VPS(32)/SPS(33)/PPS(34) 作为 extradata；Prefix/Suffix SEI（39/40）透传含 recovery_point GDR；EOS/EOB（36/37）上报流结束事件。**不支持** PACI(50)。NAL Type 47 由 ITU-T H.265 Table 7-1 定义；48-50 由 RFC 7798 §1.1.4 在 RTP 层占用。
...
**激活原则**：同时枚举 hardware async + software sync MFT 作为兜底回退；**硬件 MFT 永远 async**（Microsoft 文档明确——协议要求而非性能选择，sync 模式会 stall），激活时显式 unlock async + 用事件生成器驱动；通过 `IMFDXGIDeviceManager` 把 renderer 的 `ID3D11Device` 注入 MFT 确保同 device。
...
- `CODECAPI_AVLowLatencyMode` 启用低延时模式（禁 reorder buffer）。**Microsoft H.264 decoder 该属性用 `VT_UI4`（UINT32），其它（H.264 encoder、H.265 decoder/encoder）用 `VT_BOOL`**——MF 文档明示的实现差异（AVLowLatencyMode "Warning" 块），SetValue 时按 codec 区分类型。
...
任一 D3D11 / DXGI 调用返回 `DEVICE_REMOVED` 或 `DEVICE_RESET` 时：销毁解码器/swap chain/view 链 + ClearState + 释放 D3D11 device → **重建 DXGI factory（DEVICE_REMOVED 后 stale，必须重建）** → LUID 找回同 adapter（失败则智能重选 §7.3）→ 重建 device / MFT 或软解 / swap chain → 清空 jitter（数据依赖旧 device）→ PLI 请求新 I 帧 → 触发 device-lost / device-recovered 事件序列。
